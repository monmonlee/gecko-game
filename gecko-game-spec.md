...
## Phase 4 — 玩家自製內容與生命終章（規劃中，尚未實作）

### 4A. 自製 BGM（玩家提供音檔）
- 玩家將自行創作 BGM，檔案（mp3/ogg）放入 `public/bgm/`
- 接入點：`src/sound.js` 已有完整音訊架構與靜音開關（目前整體關閉中）
- 播放規則建議：第一次點擊畫面後開始循環播放（瀏覽器限制）；開燈／關燈可切不同曲子或濾波（關燈時低通濾波變「悶悶的」，很有在缸外聽的感覺）
- 恢復 🔊 按鈕讓玩家自行開關

### 4B. 自製封面（玩家提供圖檔）
封面會用在三個地方：
1. **開場畫面**：打開遊戲先看到封面（標題＋「進入」按鈕），做成 `#title` 覆蓋頁，放在 `index.html`，與現有的手冊/圖鑑同款覆蓋層架構
2. **分享預覽圖**：`<meta property="og:image">`，朋友在 LINE 貼連結時會顯示封面
3. **PWA icon**：替換 `public/icon-*.png`（目前是程式生成的大眼睛）

### 4C. 生命終章——「牠留下的一封信」
核心精神：**不是死亡懲罰，是一段陪伴的完成**。設計基調維持療癒，是「謝謝你」而不是「失去」。

- **老年期（elder）**：成體之後新增第四階段。色澤變淡、動作變慢、睡得更多，日記語氣變得更溫柔緩慢（「今天走到水盆邊，花了比較久。不過你看，我還是到了。」）
- **那一天**：老年期結束後某次打開遊戲——缸裡很安靜，牠最喜歡的位置空著，放著一封信 📜
- **信的內容**：由這段陪伴的真實紀錄自動生成，每個玩家的信都不一樣——
  - 一起度過的天數、吃過的蟲蟲數、上手次數
  - 牠最常睡的地點（從紀錄統計）
  - 日記裡的關鍵時刻（第一次被摸、第一次爬上手心）
  - 結尾：「我是一隻很幸福的守宮。──小波」（用玩家取的名字）
- **之後**：
  - 缸變成「紀念模式」：相簿、日記、圖鑑、那封信永久保留可翻閱
  - 可以選擇「迎接一隻新的守宮」開始新的一生（舊紀錄封存在紀念頁，不覆蓋）
- **實作位置**：`state.js` 加 elder/completed 階段與結算；信件頁為新覆蓋層 `#letter`；紀念模式是 `main.js` 的分支入口
- **待玩家決定的設計問題**：
  1. 一生多長？（真實守宮 15–25 年；遊戲建議：約真實 1 年進入老年、共約 1.5 年壽終？或更短？）
  2. 要不要在手冊裡預告「牠會陪你一段時間，然後謝謝你」？還是讓玩家自己發現？
  3. 老年開始前是否提供「暫停時間」的選項（有些玩家可能不想面對）？
